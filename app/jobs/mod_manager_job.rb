    ...
    if response&.present?
      ...
      Mod::MANAGED_MODS.each do |mod_name|
        ...
        mod_data["versions"].each do |version|
          ...
          Rails.logger.info "Registering new mod version #{version['version_number']}"
        ...
        Rails.logger.info "Done updating #{mod_name}"
      end
    end
  end
